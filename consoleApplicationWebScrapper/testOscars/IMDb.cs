...
namespace IMDb_Scraper
{
    public class IMDb
    {
        ...
        //Get IMDb URL from search results Grant heslov
        ...
        public void getData(string imdbUrl, bool GetExtraInformation)
        {

            string html = getUrlData(imdbUrl);
           // string tempID = match(@"<link rel=""canonical"" href=""http://www.imdb.com/title/(tt\d{7})/combined"" />", html);
            string tempID = match(@"<link rel=""canonical"" href=""http://www.imdb.com/event/ev0000003/(.*?)"" />", html);
            //<link rel="canonical" href="http://www.imdb.com/title/tt3748528/combined" /><meta property="og:url" content="http://www.imdb.com/title/tt3748528/combined" />

            if (!string.IsNullOrEmpty(tempID))
            {

                //string nameOfLeadingActor = match(@"<a href=""/name/nm/"", html);
                string winnerAndNomineeList = match( @"Actor in a Leading Role</h2>(.*?)</blockquote>", html);
                string tempMatch = match(@"WINNER</h3>(.*?)<h3>NOMINEES</h3>", winnerAndNomineeList);
                string startWithEndWith = match(@"^<a href=(.*)</a>$", winnerAndNomineeList);
                ArrayList sauce = matchAll(@"<a.*?>(.*?)</a>", winnerAndNomineeList);
                ArrayList redefinedSauce = matchAll(@"<a.*?>(?!<img)(.*?)</a>", winnerAndNomineeList);

              
                // This gets the Url from the a tag surrounding the actor
                string getLinkToWinnerPage = match(@"</strong><a  href=""/name/(.*?)/"">", winnerAndNomineeList);
                string getLinkToWinnerFilm = match(@"<strong><a.?>(?=[^a-z]*[a-z])(.*?)", winnerAndNomineeList);
                string testGetLinkToFilm = match(@"<strong><a href=""/title/(.*?)"" ", winnerAndNomineeList);

                string getLinks = match(@"<strong><a  href=""/title/(.*?)/"">", winnerAndNomineeList);
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (redefinedSauce[i].ToString() == "" || redefinedSauce[i] == null)
                    {
                    }
                    else
                    {

                        Winner.Add(redefinedSauce[i]);
                    }                    

                }

                NameValueCollection winnerList = new NameValueCollection();
                
                //winnerList[Winner[1].ToString()] = winnerList[0];
               

                ArrayList testRed = matchAll(@"^<div class=(.*)>(.*?)</div>", winnerAndNomineeList);      
                string nameOfFilmWhichActuallyReturnsActorsName = match(@"<a.*?href=""/title/tt.*?/"">(.*?)</a>", tempMatch);
                Regex r = new Regex(@"<a.*?href=(""|')(?<href>.*?)(""|').*?>(.*?)</a>");
                ArrayList nameOfFilm = matchAll(r.ToString(), tempMatch);

                string test = match(@"<a href=""[a-zA-Z0-9][1,1000]"">(.*?)</a>", tempMatch);

                //ArrayList nameOfFilm = matchAll(@"<strong><a.*?href=""/title/tt.*?/""></a>", tempMatch);

                //ArrayList friends = matchAll(@"<a.*?href=(""|')(?<href>.*?)(""|').*?>(?<value>.*?)</a>"",tempMatch);

                //Regex r = new Regex(@"<a.*?href=(""|')(?<href>.*?)(""|').*?>(?<value>.*?)</a>");

                //<a.*?href=""/name/.*?/"">(.*?)</a>
                //^abc(.*)xyz$
                IEnumerable finalMatch = matchAll(@"<a.*?href=""/name/nm.*?/"">(.*?)</a>", tempMatch);

                //<strong><a  href="/title/tt1663202/" >The Revenant</a>: </strong><
                // matchAll(<a  href="/name/nm0000138/">Leonardo DiCaprio</a>
                // @"<td valign=""top""><a.*?href=""/name/.*?/"">(.*?)</a>"
               // Producers = matchAll(@"<td valign=""top""><a.*?href=""/name/.*?/"">(.*?)</a>", match(@"Produced by</a></h5>(.*?)</table>", html));
               

            }

            //Id = match(@"<a rel=""canonical"" href=""http://www.imdb.com/title/(tt\d{7})/combined"" />", html);
        ...
        private void parseIMDbPage(string imdbUrl, bool GetExtraInfo)
        {
            ...
            // After the line above the information be derived from the tag below comes form Rogue One/Combined
        ...
        private string match(string regex, string html, int i = 1)
        {
            ...
            Regex regexs = new Regex(regex, RegexOptions.Multiline);

            Match match = regexs.Match(html);

            string result = match.Groups[i].Value.Trim();
